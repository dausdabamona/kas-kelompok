...
| V5 | **SEDANG** | `Javascript.html` (123 sink `innerHTML`) | Data user (nama jamaah, catatan, keterangan) dirender via `innerHTML` **tanpa escaping** (kecuali `getRiwayatTransaksiSaya` yang sudah pakai `_esc`). | XSS tersimpan: catatan berisi `<script>`/`<img onerror>` tereksekusi di perangkat pengurus. |
